...
void ofApp::update(){
    ...
    for (int j = 0; j < NUMBER_OF_RINGS; j++){
        ...
        int BANDS_DATA_PER_ITEM = NUMBER_OF_BANDS / NUMBER_OF_RINGS;
        ...
        if (surgeAmount > 0) {
            rings[j].setAcceleration(surgeAmount);
        }
        // why did I set z
        rings[j].setZ(ringZ);
...
void ofApp::fftDraw(){
    ...
    for (int b = 0; b < NUMBER_OF_BANDS; b++) {
        ...
        ofSetColor(r, g, b, 80);
        ofDrawRectangle(100 + bandwidth * b, ofGetHeight() * .9,
                        bandwidth,
                        -(fftSmoothed[b] * (ofGetHeight()/2)));
    }
}
